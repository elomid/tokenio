...
let sessionY: CGFloat = 12   // top bar (bottom-left origin)
let weeklyY: CGFloat = 3     // bottom bar
...
func makeIcon(sUsage: Double, sTime: Double, wUsage: Double, wTime: Double) -> NSImage {
    ...
    img.lockFocus()
    drawBar(x: barX0, y: sessionY, w: barW, h: barH,
            corner: barCorner, fillFrac: sUsage / 100, tickFrac: sTime / 100,
            bgAlpha: iconBgAlpha)
    drawBar(x: barX0, y: weeklyY, w: barW, h: barH,
